export const LOGIN = '[AUTH] LOGIN'
export const REGISTER = '[AUTH] REGISTER'
export const LOGOUT = '[AUTH] LOGOUT'
export const UPDATE_PROFILE = '[AUTH] UPDATE_PROFILE'
export const UPDATE_AVATAR = '[AUTH] UPDATE_AVATAR'
export const CHECK_AUTH = '[AUTH] CHECK_AUTH'
export const FAVORITE_TOGGLE = '[FAVORITE] FAVORITE_TOGGLE'
export const FETCH_POST = '[POST] FETCH_POST'
export const LISTEN_POST = '[POST] LISTEN_POST'
export const LISTEN_ONLINE_USER = '[ONLINE] LISTEN_ONLINE_USER'
